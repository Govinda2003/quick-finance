...
function parseRss(xmlText: string): Array<{ title: string; link: string; description: string; pubDate: string; source: string }> {
  ...
  for (const itemXml of itemMatches) {
    ...
    const description = extractTag("description").replace(/<[^>]*>?/gm, ''); // strip HTML tags
    const pubDate = extractTag("pubDate");
    
    let source = "Live News Feed";
    if (link) {
      try {
...
export async function POST(request: Request) {
  try {
    ...
    // 2. Check if live news API or RSS feeds are configured
    const liveNewsApiConfigured = !!process.env.NEWS_API_KEY;

    if (!liveNewsApiConfigured) {
      // Branch Flow: Not Configured -> Generate Dynamic Demo Edition
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Live news API not configured yet. Showing demo edition.",
          edition: demoEdition,
        },
        { status: 200 } // Send 200 so the frontend can receive the edition payload properly
      );
    }

    // Branch Flow: Configured -> Aggregate Live Feeds and APIs
    const rawArticles: Array<{ title: string; link: string; description: string; pubDate: string; source: string }> = [];

    // Define standard feeds
    const feeds = [
      "https://news.google.com/rss/search?q=finance+OR+fintech+OR+AI+OR+investing+OR+macro+OR+consulting&hl=en-US&gl=US&ceid=US:en",
    ...
    try {
      const newsApiRes = await fetch(
        `https://newsapi.org/v2/everything?q=finance+OR+fintech+OR+AI+OR+investing+OR+macro+OR+consulting&sortBy=publishedAt&apiKey=${process.env.NEWS_API_KEY}`,
        { signal: AbortSignal.timeout(5000) }
      );
      if (newsApiRes.ok) {
        const json = await newsApiRes.json();
        if (json.articles) {
    ...
    const filterKeywords = [
      ...
      "dxy", "market", "stocks", "sec", "treasury", "acquisition", "merger", "pe", "vc"
    ];
    ...
    const preferredSources = [
      ...
      "economictimes", "moneycontrol", "livemint", "cnbc", "techcrunch", "theken", "mckinsey",
      "bcg", "bain", "hbr"
    ];
    ...
    const scoredArticles = uniqueArticles.map((art) => {
      ...
      if (preferredSources.some((src) => sourceLower.includes(src) || art.link.includes(src))) {
        score += 20;
      }
    ...
    // Fall back to template if we don't have enough articles
    if (sorted.length < 5) {
      throw new Error("Insufficient live articles collected to build edition.");
    }

    // 4. Generate the structured NewspaperEdition
    ...
    while (aiFintechList.length < 2 && idx < sorted.length) {
      ...
      const isAiFin = /ai|fintech|payment|tech|openai|nvidia|gpu|model|llm/i.test(art.title + " " + art.description);
      if (isAiFin) {
    ...
    // Fill in from remaining if not enough AI articles found
    idx = 1;
    while (aiFintechList.length < 2 && idx < sorted.length) {
      const art = sorted[idx];
    ...
    // Smart Reads (next 2 academic or research papers)
    ...
    // Fill from base mock smartReads if not enough
    ...
    console.error("Manual generate handler error:", error);
    return NextResponse.json(
      {
        error: error?.message || "Internal server error occurred during edition generation.",
      },
      { status: 500 }
    );
  }
