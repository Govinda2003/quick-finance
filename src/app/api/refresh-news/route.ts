...
function generateTakeawaysFromTitle(title: string): string[] {
  ...
  const clean = title.replace(/ - [^-]+$/, "").replace(/\s+(is said to|are said to|said to|soars?|jumps?|drops?|falls?|rises?|reports?|says?|according to|ahead of|after|amid|as|following).*/i, "").trim();

  if (/\b(fed|rate|inflation|yield|central bank|monetary)\b/i.test(t)) return ["Central bank policy is shifting on rates and liquidity.", "Rate decisions affect borrowing costs, equity valuations, and currency strength.", "Watch the next inflation print and forward guidance from policymakers."];
  if (/\b(ipo|listing|public offering|debut)\b/i.test(t) && !/ahead of ipo|before ipo|prior to ipo/i.test(t)) return [`${clean.split(" ").slice(0,6).join(" ")} is preparing to go public.`, "IPO activity signals investor appetite for new equity in this sector.", "Watch the pricing, valuation multiple, and first-day trading performance closely."];
